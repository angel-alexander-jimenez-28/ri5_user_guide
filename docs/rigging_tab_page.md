...
![Ri5_Guide_Images](images/Rigging_Tools.png)

it is important to note that when we've pressed **Make Car Ready For Rig** on the Clean Tab Earlier 
the addon keep the <span class="yellow">Yellow Collection</span> selected as the active collecion also if we take look a little bit closer we can see that the objects inside are selected and the active one is the object that represent the Car's body

as shown below:<br>
![Ri5_Guide_Images](images/Rigging_Tools_Active_Collection.png)

this selection is done by the addon for us just to keep the workflow a little bit more user friendly, but why?

#### Create Car Rig

because there is a behavior when we press **Create Car Rig** if there isn't nothing selected like it is show in the image above the operator will creates the rig outside of the <span class="yellow">Yellow Collection</span> and the Bones of the rig will not be positioned on each car pieces by default. 

as shown below:<br>
![Ri5_Guide_Images](images/Rigging_Tools_Un_selected_when_rigging.gif)
...
as shown below:<br>
![Ri5_Guide_Images](images/Rigging_Tools_Controls_Rig.gif)
